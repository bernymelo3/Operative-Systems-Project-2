...
static pthread_mutex_t destroy_all_closed;
static pthread_cond_t files_closed;
...
int tfs_init() {
    ...
    if (pthread_cond_init(&files_closed, NULL) != 0) {
        return -1;
    }    
    if (pthread_mutex_init(&destroy_all_closed, 0) != 0)
        return -1;
    /* create root inode */
...
int tfs_destroy_after_all_closed() {
    if (pthread_mutex_lock(&destroy_all_closed) != 0)
        return -1;

    destroy_all = 1;
    while (open_files > 0) {
        pthread_cond_wait(&files_closed, &destroy_all_closed);
    }
    
    if (pthread_mutex_unlock(&destroy_all_closed) != 0)
        return -1;
    tfs_destroy();
...
int tfs_close(int fhandle) {
    ...
    if (open_files == 0) {
        pthread_cond_broadcast(&files_closed);
    }
